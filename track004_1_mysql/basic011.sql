...
where ( sal = ( select max(sal) from emp where deptno=10) and deptno=10 )
	or ( sal = ( select max(sal) from emp where deptno=20) and deptno=20 )
    or ( sal = ( select max(sal) from emp where deptno=30) and deptno=30 )
order by empno asc;
-- where  동일한 급여 받는 사람 = 각 부서별 최고 급여와 (deptno in 10,20,30) 
...
select empno, ename, job, mgr, hiredate, sal, comm, d.deptno
from emp e join dept d on e.deptno = d.deptno
...
-- --------------------------------------------------------
-- -- 연습문제-12
-- -- emp 테이블에서 ANY를 이용하여 30번 부서의 최고 급여보다 작은 급여를 받는 사원정보를   조회하시오.
-- -- ※ ANY , SOME 연산자는 서브쿼리가 반환한 여러결과값 중
-- -- 메인쿼리와 조건식을 사용한 결과가 하나라도 TRUE라면 메인쿼리조건식을 TRUE로 반환해주는 연산자
-- +-------+--------+----------+------+------------+------+------+--------+
-- | empno | ename  | job      | mgr  | hiredate   | sal  | comm | deptno |
...
select *
from emp
where sal < any( select max(sal) from emp where deptno=30 group by deptno)  -- 30번 부서의 최고 급여
order by sal asc;

-- step1)
select sal from emp where deptno=30 order by sal asc; -- 950 1250 1250 1500 1600 2850
-- step2) in =any =some
-- 			sal < any( 950 1250 1250 1500 1600 2850. );
-- 			sal > any( 950 1250 1250 1500 1600 2850  );
