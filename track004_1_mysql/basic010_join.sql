...
alter table join_userinfo modify no int not null default 0;
...
delete from join_userinfo where no=10;
...
select * from emp, dept order by empno;
-- emp와 dept 테이블에 empno가 있는 모든 데이터 출력
-- 조인 조건 없으면 모두 조회 (카다시안곱 - 모든조합)
-- +-------+--------+-----------+------+------------+------+------+--------+--------+------------+----------+
...
-- depno 필드가 emp, dept 둘다 있어서... 겹치는 필드 소속 채우기
...
-- where     사원.sal   between  SALGRADE의 최소급여   and  최대급여사이이 있는  (연결조건)
-- order by  직속상관의 번호를 오름차순으로

						-- 직속상관의번호(소속.empno)	직속상관의이름(소속.ename)
...
select e1.empno, e1.ename, e1.mgr, e2.empno`직속상관번호`, e2.ename`직속상관이름`
from emp e1, emp e2
...
-- select   사원소속의.EMPNO , 사원소속의.ENAME  , 사원소속의.MGR , 직송상관의.empno  `MGR_EMPNO`  , 직송상관의.ename  `MGR_ENAME` 
-- from     emp  sawon  right join  emp  mgr
-- on       사원소속의.mgr = 직송상관의.MGR_EMPNO 
-- order   by   직송상관의.EMPNO  asc; 
...
order by deptno, empno;		-- deptno 필드 겹치는 부분 1번 출력
...
select e.deptno, job, empno, ename, sal
from emp e join dept d
on e.deptno=d.deptno
where sal>2000;
...
-- select DEPTNO | DNAME      | EMPNO | ENAME  | MGR  | SAL  | DEPTNO | LOSAL | HISAL | GRADE | MGR_EMPNO | MGR_ENAME
select * from emp, dept, salgrade;

select d.DEPTNO, d.DNAME, e.EMPNO, e.ENAME, e.MGR, e.SAL, e.DEPTNO,
		s.LOSAL, s.HISAL, s.GRADE, mgr.empno`MGR_EMPNO`, mgr.ename`MGR_ENAME`
from emp e right join dept d on (e.deptno = d.deptno)
			left join salgrade s on (e.sal between s.loasl and s.hisal)
