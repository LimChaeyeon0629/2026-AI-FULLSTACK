...
-- 1. 시스템의 현재 시각 조회
select now();			-- '2026-05-26 14:37:03'
select current_time();	-- '14:37:42'
...
select date_format( '2026-05-26 14:37:03', "%Y-%m-%d" );
...
select datediff( "2026-05-26", "2026-05-25" );	
select timestampdiff( hour, "2026-05-25", "2026-05-26" );

-- now(), weekdat (0=월, ... 6=일), date_format(날짜, "%Y-%d-%m %H:%i:%s"), date_add
...
-- alter table date_userinfo modify int not null auto_increment primary key;	-- 기본키 추가

create table date_userinfo1 (
...
insert into date_userinfo1 (no, name, age, date) values (1, 'aaa', 11, '2022-12-30 00:00:00');
insert into date_userinfo1 values (2, 'bbb', 22, '2022-11-30 00:00:00');
insert into date_userinfo1 values (3, 'ccc', 33, '2022-10-30 00:00:00');
insert into date_userinfo1 values (4, 'ddd', 44, '2022-09-30 00:00:00');
...
-- +------+---------------------+
select name, date from date_userinfo1;
...
select now();				-- 오늘 날짜
select weekday( now() );	-- 1=화요일 0(월)~6(일)
select 6- weekday(now());	-- 일요일6 - 현재요일값1

select date_add( now(), interval 5 day );					-- 결론 : 수동으로 5월 31일 계산
select date_add( now(), interval 6-weekday(now()) day );	-- 가장 가까운 오는 일요일
select date_add( now(), interval -2 day );					-- 수동으로 5월 24일 계산
select date_add( now(), interval (6-weekday(now()))-7 day );	-- 가장 가까운 지난 일요일
select date_add( now(), interval -( weekday(now()) +1) day );	-- 가장 가까운 지난 일요일
-- 								 -( 1			   +1)			-- -2
...
-- 1보다 크다, 작다
select no, if(no>1, '크다', '작다.')`1크다, 작다`
from control;

-- case
select no, case		-- case ~ end
			when no=1 then '1이다'
			when no=2 then '2이다'
			when no=3 then '3이다'
            else '1, 2, 3이 아니다'
...
-- |             3 |
-- +---------------+
select count( if(sns='n', 1, null) )`sns 수신거부수`
from if_userinfo;

select count(*)`sns 수신거부수`
from if_userinfo
where sns='n';
...
select ename, job, case
					...
					when job!='CLERK'or'SALESMAN' then '사원'
					end`JOB2`
from emp;
...
-- (1) users 라는 테이블 만들기   / (2) sql  -  회원가입 sql,  로그인 sql 찾기~!
create table users (
	uno int not null auto_
);