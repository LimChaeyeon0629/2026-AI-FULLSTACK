...
select 	ban, count(*)
from	group_userinfo;					-- error !

select ban from group_userinfo; 		-- 6줄 (집계안된 일반)
select count(*) from group_userinfo;	-- 1줄 (집계)
...
select ban, count(*) * avg(kor+eng+math)/3`평균총점`
from group_userinfo
group by ban
having count(*) * avg(kor+eng+math)/3 >=80;

-- select ban, count(*), avg(kor+eng+math)/3
-- from group_userinfo
-- group by ban;
...
-- select ban, max(kor), avg(kor+eng+math)/3`평균점수`
-- from group_userinfo
-- group by ban;

select ban, avg(kor+eng+math)/3`평균점수`
from group_userinfo
group by ban
order by avg(kor+eng+math)/3 desc
limit 0,1 ;
...
select * from emp;
select count(*)`전체레코드`, count(empno)`모든데이터`, count(mgr)`null빼고계산`
from emp;
...
select count(comm)`추가 수당을 받는 사원수`
from emp;
...
select deptno, job, count(job)`사원수`, max(sal)`최고급여`, sum(sal)`급여합`, avg(sal)`평균급여`
from emp
group by deptno, job
order by deptno asc;