...
→ 서버가 완성된 HTML 을 만들어서 클라이언트(브라우저)에게 보낸다
기술스택     : BOOT + security + thymeleaf + oracle + mybatis
            (backend + react)
- 렌더링주체 : 서버
- 초기로딩   : 빠름
- 화면전환   : 새로고침
...
#### 4. [config] - db.js             # Oracle DB 설정
#### 5. [models] - users.js          # 사용자 DB 모델 및 쿼리 함수
#### 6. 모델함수 테스트
