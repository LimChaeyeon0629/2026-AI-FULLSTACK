...
const api = axios.create({
    ...
    // refreshToken이 HttpOnly 쿠키에 저장이 되어 있으면 자동으로 포함 필요
...
api.interceptors.request.use(
    (config) => {                            // SSR (타임리프 등...) 
        if (typeof window !== "undefined") { // CSR (클라이언트+서버) 환경에서만 localStorage 접근
