...
public class AppUser {
	...
	// 1. mappedBy="user"				post 엔티티에 있는 user 필드와 연결 - 읽기가능 / 수정 x
	// 2. cascade = CascadeType.ALL		AppUser 변화 (생성, 수정, 삭제 등)와 연결된 Post 에 반영
	// 3. orphanRemoval = true			유저탈퇴 시 글들이 깔끔하게 삭제
	...
	private List<Follow> following = new ArrayList<>();		// 팔로잉: 내가 팔로우한 사람들
	@OneToMany(mappedBy = "followee", cascade = CascadeType.ALL, orphanRemoval = true)
	private List<Follow> followers = new ArrayList<>();		// 팔로워: 나를 팔로우한 사람들
