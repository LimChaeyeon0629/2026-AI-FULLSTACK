...
2. jsp에서 다른페이지로 이동
	1) sendRedirect -  브라우저 url을 변경o	, request/response 객체가 유지안됨
	2) RequestDispatcher -  위임(요청전달)
		브라우저 url을 변경x	, request/response 객체가 유지	 됨
		</pre>
		<%
		//ver-1 알림창 처리안됨.
		// out.println("<script> alert('안녕?'); </script>");	// alert 창 출력불가
		...
		//ver-2
		// jsp013_3.jsp.
		// request.getRequestDispatcher("milks.jsp") 이용해서 여기서 처리해주세요!
		// response.sendRedirect("jsp013_3.jsp"); //013으로 경로 위임
		%>
...
1. application (웹 애플리케이션이 실행되고 있는 동안, 서버가 꺼질 때까지 유지)
		> session (특정 브라우저와 연결된 세션, 브라우저 종료 시 소멸)
			> request (요청: form, a 태그 요청)
				> page (해당 jsp 페이지에서만)
