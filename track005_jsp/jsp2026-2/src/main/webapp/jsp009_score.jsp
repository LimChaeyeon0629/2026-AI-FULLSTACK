...
<body>
	<div class="container card  my-5">
		...
		<pre class="my-3">
		 처리할 경로 : jsp009_result.jsp 		=> action
		 주소표시창줄에 노출 X					=> method : post
		 보관용기이름 : kor / eng / math 		=> name=""
	...
	<script>
		function check() {
			...
			if(kor.value.trim()="") {
				alert("kor이 빈칸입니다. 채워주세요.");
				kor.focus();
				return false;
			}
			if(eng.value.trim()="") {
				alert("eng이 빈칸입니다. 채워주세요.");
				eng.focus();
				return false;
			}
			if(math.value.trim()="") {
				alert("math이 빈칸입니다. 채워주세요.");
				math.focus();
				return false; 	// 폼 제출 막기 (action="jsp009_result.jsp" 로 못 넘어가게)
				// return true;	// 폼 제출
			}
		}
	</script>
