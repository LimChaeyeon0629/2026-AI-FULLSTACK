...
//	int age;
	abstract void eat();   // 추상메서드 { } 바디 구현부가 없음.
...
public class Abstract001 {
	public static void main(String[] args) {
		// 1. abstract: 일반클래스 + 설계
//		Animal ani = new Animal(); // new메모리빌리고객체생성, Animal()불러서초기화, ...
								   // 					  → { } 구현부 없어서 성립 x
		// abstract void eat();
		...
		int cnt = 0;
		for( Animal a : arr ) { 
			a.name = "ani" + ++cnt;		// 각각 인스턴스에 이름
			a.eat();					// eat() 자식 꺼 오버라이드 (최신 기능)
		}
	}
}

/*
Abstract (is A) 일반클래스(인스턴스변수) + 설계
	고양이도 동물이다
	개도	   동물이다
...
/*
1. 추상화 (abstract - 강제성, 무조건 구현해야함)
- 실체화된 객체들 간 공통된 특성을 추출
