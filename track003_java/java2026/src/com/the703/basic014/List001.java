...
//		list3.add(3.14);  // Object obj = 3.14
		...
		// Step4. 	add(추가), get(가져오기), size(갯수), remove(삭제), contains(포함 여부)
...
2. 콜렉션 프레임워크
- [배열]의 단점을 개선한 클래스, 객체(클래스) 저장가능
- 동적배열

※ 배열) 같은 타입만 묶어서 저장가능, 배열 생성 시 크기를 지정해서 사용, 추후 변경 불가능
   String [] arr = new String[3];

3. 핵심 인터페이스
1) List - 기차 ( 순서 o, 중복 허용 o )
		  add, get, size, remove, contains

2) Set  - 주머니 ( 순서 x, 중복 허용 x )
		  add, get(x) → (향상된 for or Iterator), size, remove, contains
