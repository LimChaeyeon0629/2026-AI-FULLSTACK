...
public class Set001 {
	public static void main(String[] args) {
		...
		set2.add( new Candy("츄파춥스", 200) );
		set2.add( new Candy("츄파춥스", 200) );
		set2.add( new Candy("청포도 사탕", 4500) );
		set2.add( new Candy("아이셔", 1500) );
		
		System.out.println(set2);
		System.out.println(set2.size());
		System.out.println(set2.remove( new Candy("츄파춥스", 200)) ? "냠냠" : "못 먹었어" );
		...
		Integer i1 = 1;  // 부품객체 = 기본값 (기본값 1을 받아서 객체로 인식)
		int		i2 = i1; // 기본값 = 부품객체
		float 	 f = i1.floatValue();
//		float 	f2 = i2.floatValue(); // 기본값은 부품 사용 불가능
