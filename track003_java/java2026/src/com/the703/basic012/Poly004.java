...
class TestA4 extends Object {
	int a=10;
	@Override public String toString() { return "TestA4 [a=" + a + "]"; } // @Override(오버라이딩) 자식에 맞게 수정
} // end class
...
public class Poly004 {
	public static void main(String[] args) {
		...
		// 1. ta { a=10, toString }  = 1번지 { a=10, toString }
		
		TestB4 tb					 				 = new TestB4();
		// 2. tb { b=20, @toString } - { a=10, ... } = 2번지 { b=20, @toString } - { a=10, ... }
		
		/* 오류
		tb = (TestB4)ta;
		// { b=20, @toString } - { a=10, ... } = 1번지 { a=10, toString }
		System.out.println(tb); // TestB4 [b=20]
		System.out.println(tb.b);
		System.out.println(tb.a);
		*/

		ta = new TestB4(); // 해결방안: ta에 자식 생성자를 호출한 적이 있어야 한다
...
3. 자식은 부모를 담을 수 있다. (다운캐스팅 -> 컴파일러에서 오류)
--------------------------------------------------
<<class>> Animal 	[ 이름, 나이 / 먹기, 자기, 배변 ]
			↓
<<class>> Cat		[ 동물등록증 / 꾹꾹이 하기 ]
