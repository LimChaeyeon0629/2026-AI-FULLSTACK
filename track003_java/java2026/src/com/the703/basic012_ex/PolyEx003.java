...
{ name="청소로봇"; 		{ name="요리로봇";			{ name="전투로봇";
@work(), vacuum() }		@work(), cook() }			@work(), attack() }
...
abstract class Robot { // abstract 추상 메서드(일반화)
    String name = "기본로봇";
    public Robot() { super(); }
...
//        System.out.println(robots[1].name); // 기본로봇
        ...
        ((FightingRobot)robots[2]).attack(); // 타입캐스팅 메서드 호출

        /* 선언
        FightingRobot fiR = new FightingRobot();
        fiR.attack(); */
	}
}