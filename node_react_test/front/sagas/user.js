...
export function checkNicknameApi(nickname) {
    return client.get(`/user/check-nickname?nickname=${encodeURIComponent(nickname)}`);
}
...
export function* checkNickname(action) {
    try{
        ...
        yield put({type:CHECK_NICKNAME_FAILURE,
                    checkNicknameError: err.response?.data || err.message
        });
    }
}
...
export function searchUserApi(keyword) {
    return client.get(`/user/search-user?keyword=${encodeURIComponent(keyword)}`);
}
...
export function* searchUser(action) {
    try {
        ...
        yield put({type:SEARCH_USERS_FAILURE,
                    searchUsersError: err.response?.data || err.message
        });
    }
}
