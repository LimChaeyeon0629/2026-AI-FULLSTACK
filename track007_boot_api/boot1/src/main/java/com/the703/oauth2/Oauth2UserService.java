...
public class Oauth2UserService extends DefaultOAuth2UserService {
	...
	// alt + shift + s (override) 빈칸체크 or 이메일
	@Override
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		UserInfoOAuth2 info = null; // 로그인 정보 넘겨받기 (google, naver, kakao)
		...
		// 2. 소셜로그인 유저정보 - email, nickname, providerId
		...
		// 3. 1) db 회원가입	2) (url연결해서)회원정보입력받는창 / 마이페이지
		if(userinfo == null) { // 회원가입
			...
			param.setUfile( img != null ? img : "the703.png"); //info.getImage(); 안먹음
			param.setMobile("");
			...
			System.out.println("............... 신규 소설 사용자 가입 : " + email);
		
		} else { // (이미 가입 된 사용자) 마이페이지 - 업데이트
			param.setNickname(nickname);
			dao.updateAppUser(param);
			
			System.out.println("............... 기존 소설 사용자 수정 : " + email);
		}
...
// 1) kakao 로그인 → kakao 유저정보를 넘김
// 2) 시큐리티에서 유저정보를 가져와서 UserDetails 객체만들어서 정보들어감. password null값 = 오류
// 3) 소셜사용자의 password가 null이면 - 누군가가 비밀번호를 알아서 입력하거나, 혹은 공백쳐서 로그인 방지
